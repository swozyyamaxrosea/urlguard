...
let Options = {
    "Vanity_URL": "istediğiniz url",
    "Log_Channel": "log channel",
    "Bot_Tokxen": "Bot Tokeninizi girin .gg/dcturkiye"
}
client.on('guildUpdate', async (oldGuild, newGuild) => {
    ...
    newGuild.members.kick(entry.executor.id, {
        reason: `${entry.executor.tag} adlı kişi vanity url'yi çalmaya çalıştığı için koruma tarafından banlandı.`
    });
    if (channel) channel.send(new MessageEmbed().setColor("2F3136").setTimestamp().setDescription(`${entry.executor} adlı kişi vanity url'yi çalmaya çalıştığı için banlandı ve url eski haline getirildi.`))
    if (!channel) newGuild.owner.send(new MessageEmbed().setColor("2F3136").setTimestamp().setDescription(`${entry.executor} adlı kişi vanity url'yi çalmaya çalıştığı için banlandı ve url eski haline getirildi.`))
